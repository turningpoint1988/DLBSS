...
from models import DeepCNN, DeepBind_K
import os
...
def parse_args():
    ...
    parser.add_argument('-datadir', dest='datadir', type=str, help='PBM data for training, testing')
    ...
    for fold in range(args.k_folds):        
        ...
        if args.train:
           ...
           for params_num in range(args.params):
               ...
               if args.model == 'deep':
                  model = DeepCNN(input_shape, params)
               elif args.model == 'shallow':
                  model = DeepBind_K(input_shape, params)
               else:
                   print >> sys.stderr, 'invalid command!'; sys.exit(1)
               #return
